...
from .modeling_pretrain import itpn_base_3324_patch16_224, itpn_large_2240_patch16_224
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: NestedTensor, bool_masked_pos=None, aux_shapes=None):
        xs = self[0](tensor_list, bool_masked_pos=bool_masked_pos, aux_shapes=aux_shapes)
...
def build_backbone(args):
    ...
    if args.backbone in ['resnet50', 'resnet101']:
        ...
        if "backbone_dir" in args:
            ...
            print(str(_tmp_st_output))
        bb_num_channels = backbone.num_features[0:len(return_interm_indices)]
    elif args.backbone in ['ViT_base']:
        backbone = vit_base_patch16(args.backbone)
        # return_interm_indices = [0, 1, 2,3]
        if "backbone_dir" in args:
            pretrained_dir = args.backbone_dir
            checkpoint = torch.load(pretrained_dir, map_location='cpu')['model']
            ...
            _tmp_st_output = backbone.load_state_dict(_tmp_st, strict=False)
            print(str(_tmp_st_output))
        bb_num_channels = [256, 256, 256, 256]
    elif args.backbone in ['Fast_iTPN_large']:
        backbone = itpn_large_2240_patch16_224(args.backbone)
        # return_interm_indices = [0, 1, 2,3]
        if "backbone_dir" in args:
            pretrained_dir = args.backbone_dir
            checkpoint = torch.load(pretrained_dir, map_location='cpu')['module']
    ...
    # print("num_channels:", model.num_channels)
