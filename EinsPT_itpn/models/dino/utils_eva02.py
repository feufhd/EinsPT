...
class RescaleInput(nn.Module):
    def __init__(self,padding: Union[int, tuple, str] = 'corner',
                 kernel_size: int = 16,
                 stride: int = 16,
                 dilation: int = 1,):
        super().__init__()
...
#                                          stride=4)
        ...
        '''
         x = x.view(
                    B, -1,
                    self.patches_resolution[0], self.inner_patches,
                    self.patches_resolution[1], self.inner_patches,
                ).permute(0, 2, 4, 3, 5, 1).reshape(B, self.num_patches, self.inner_patches, self.inner_patches, -1)
        '''
        x = x.view(
            B, -1,
            patches_resolution[0], self.inner_patches,
...
class iTPN(nn.Module):
    def __init__(
            self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=512, mlp_depth1=3,
            mlp_depth2=3, depth=24, num_heads=8, bridge_mlp_ratio=3., mlp_ratio=4., qkv_bias=True, qk_scale=None,
            drop_rate=0., attn_drop_rate=0., drop_path_rate=0.0, norm_layer=nn.LayerNorm, init_values=None,
            use_abs_pos_emb=True, use_rpe_pos_bias=False, use_shared_rel_pos_bias=False,
            use_decoupled_rel_pos_bias=False, use_mean_pooling=True, init_scale=0.001, patch_norm=True,
            use_checkpoint=True, stop_grad_conv1=True,use_rel_pos_bias=False,
            postnorm=False,
            subln=False,
            xattn=False,
            swiglu=False,
        ...
        self.patch_embed = PatchEmbed(
            ...
            norm_layer=norm_layer if self.patch_norm else None, stop_grad_conv1=stop_grad_conv1)
        ...
        self.blocks.extend([
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=bridge_mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=next(dpr), norm_layer=norm_layer,
                init_values=init_values, window_size=window_size, use_decoupled_rel_pos_bias=use_decoupled_rel_pos_bias,
                postnorm=postnorm,
                subln=subln,
