    ...
    def __init__(
        ...
        if window_size:
            if use_decoupled_rel_pos_bias:
                ...
                relative_position_index = relative_position_index[1:, 1:]
...
class iTPN(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
                 num_heads=8, bridge_mlp_ratio=3., mlp_ratio=4., qkv_bias=True, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.0, init_values=0.1, attn_head_dim=None,
                 norm_layer=nn.LayerNorm, patch_norm=False, use_checkpoint=False, use_mean_pooling=False,
                 init_std=0.02,
                 init_scale=0.01,
                 cls_token=False,
                 grad_ckpt=False,
                 ...
                 swiglu=False,
                 naiveswiglu=True,
                 xavier_normal_init=False, **kwargs):
        super().__init__()
        self.img_size = img_size
        ...
        path = '/root/data/checkpoints/fast_itpn.pth'
        msg = self.load_state_dict(torch.load(path)['model'], strict=False)
        print(msg)

        self.conv = nn.Conv2d(embed_dim, embed_dim * 2, 3, 2, 1)
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks[:-self.num_main_blocks]:
            if isinstance(blk, ConvPatchMerge):
                outs.append(x)
        ...
        outs.append(x.transpose(1, 2).view(B, -1, Hp, Wp))
        outs.append(self.conv(outs[-1]))
