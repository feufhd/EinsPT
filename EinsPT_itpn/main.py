...
from engine_pt import train_one_epoch_pt
from timm.data import create_dataset, create_loader, resolve_data_config, Mixup

from models.dino.clip_wrapper import CLIPWrapper
from models.dino.create_resnet import resnet50
...
def main(args):
    ...
    if args.pt:
        teacher = CLIPWrapper(
            clip_model='ViT-L/14',
            download_root='/path/to/save/CLIP-L/',
        )
        teacher.eval().to(torch.device('cuda'))
    # teacher = None
    ...
    elif args.CosineAnnealingLR:
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=len(data_loader_train)*args.epochs, eta_min=args.lr/100)
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.pt:
            train_stats = train_one_epoch_pt(
                model, teacher, criterion, data_loader_train, optimizer, device, epoch,
